...
// examples/bullmq for the same decorator, and examples/prisma for the
// other "no decoratable entity class" adapter.
//
// The in-memory adapter has no entity class at all (see docs/architecture.md
// §4, "Two-layer adapters"), so column capabilities here can only come from
// the Resource Builder (`.sortable()`/`.searchable()`/`.filterable()` below)
// — never from `@Sortable()`/`@Searchable()`/`@Filterable()` property
// decorators, which need a real decoratable entity class. See
